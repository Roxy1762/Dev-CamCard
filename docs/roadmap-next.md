...
- 近期已完成第一步：12 张机制牌主链路接通，并补齐 `nextBoughtCardToDeckTop` 对预约购买的覆盖。
- 下一步重点从“能触发”转到“值得构筑”：
  - 观察 `hasReservedCard` / `hasScheduledCard` / `hasVenue` 三类条件牌在中期回合的稳定触发率。
  - 评估 `gainFaceUpCard(deckTop)` 与安排节奏的联动收益是否过高或过低。
